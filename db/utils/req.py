...
from db.utils.profile import get_profile_by_user_id, get_profile_by_id, get_profile_by_user_id_with_user_req
...
async def get_answers_on_req_from_user_profile(profile_id: int, session: AsyncSession):
    return (await session.execute(select(ProfileReqBack).filter(ProfileReqBack.profile_id == profile_id)))\
        .scalars().all()
